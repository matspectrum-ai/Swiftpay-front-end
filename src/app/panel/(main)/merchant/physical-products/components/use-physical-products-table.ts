...
export interface PhysicalProductsTableFilters {
...
interface UsePhysicalProductsTableProps {
	merchantId: string;
...
export function usePhysicalProductsTable({
	merchantId,
	initialFilters,
}: UsePhysicalProductsTableProps) {
	const router = useRouter();

	const [filters, setFilters] = useState<PhysicalProductsTableFilters>(initialFilters);
	...
	const updateFilters = useCallback(
		(newParams: Partial<PhysicalProductsTableFilters>) => {
			setFilters((prev) => {
	...
	const goToNew = useCallback(() => {
		router.push(Routes.panel.merchant.physicalProductsUpsert('new'));
	}, [router]);

	const goToEdit = useCallback(
		(productId: string) => {
			router.push(Routes.panel.merchant.physicalProductsUpsert(productId));
		},
		[router],
	);
