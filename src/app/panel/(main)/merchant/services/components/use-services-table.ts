...
export interface ServicesTableFilters {
...
interface UseServicesTableProps {
	merchantId: string;
...
export function useServicesTable({
	...
	const [filters, setFilters] = useState<ServicesTableFilters>(initialFilters);
	const [services, setServices] = useState<Paginated<MinimalProductData>>(defaultPaginated);
	...
	useEffect(() => {
		...
		Promise.all([
			...
			if (!cancelled) {
				setServices(servicesRes?.data ?? defaultPaginated);
	...
	const updateFilters = useCallback(
		(newParams: Partial<ServicesTableFilters>) => {
			setFilters((prev) => {
	...
	const confirmDelete = useCallback(async () => {
		...
		if (res?.error) {
			toast('Erro ao excluir', {
				description: res.error.message ?? 'Erro ao excluir serviço',
				variant: 'danger',
			...
			toast('Serviço excluído', {
				description: res?.message ?? 'Serviço excluído com sucesso',
				variant: 'success',
	...
	const goToNew = useCallback(() => {
		router.push(Routes.panel.merchant.servicesUpsert('new'));
	}, [router]);

	const goToEdit = useCallback(
		(productId: string) => {
			router.push(Routes.panel.merchant.servicesUpsert(productId));
		},
		[router],
	);
