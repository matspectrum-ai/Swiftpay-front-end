...
import { AddCircleIcon, Add01Icon, CancelCircleIcon } from '@hugeicons/core-free-icons';
			...
			<Modal.Container size="sm" placement="center" scroll="outside">
				<Modal.Dialog className="max-w-sm">
					...
					<Modal.Header>
						...
						<p className="text-sm text-muted">Crie uma categoria para organizar seus serviços</p>
					</Modal.Header>
					<Form action={formAction} validationErrors={state.error ? { name: state.error } : undefined}>
						<Modal.Body>
							<TextField variant="secondary" name="name" isRequired>
								...
								<Input variant="secondary" placeholder="Ex: Consultoria, Assinatura..." autoFocus />
								<FieldError />
							</TextField>
						</Modal.Body>
						<Modal.Footer>
			...
			{!disabled && (
				<div className="flex justify-end">
					<Button
						variant="secondary"
						size="sm"
						onPress={() => setIsCreateCategoryOpen(!isCreateCategoryOpen)}
					>
