	...
	return (
		<ChartContext.Provider value={{ config }}>
			<div
				...
				data-chart={chartId}
				className={cn(
					"[&_.recharts-cartesian-axis-tick_text]:fill-muted [&_.recharts-cartesian-grid_line[stroke='#ccc']]:stroke-border/50 [&_.recharts-curve.recharts-tooltip-cursor]:stroke-border [&_.recharts-dot[stroke='#fff']]:stroke-transparent [&_.recharts-layer]:outline-hidden [&_.recharts-polar-grid_[stroke='#ccc']]:stroke-border [&_.recharts-radial-bar-background-sector]:fill-muted [&_.recharts-rectangle.recharts-tooltip-cursor]:fill-muted [&_.recharts-reference-line_[stroke='#ccc']]:stroke-border [&_.recharts-sector[stroke='#fff']]:stroke-transparent [&_.recharts-sector]:outline-hidden [&_.recharts-surface]:outline-hidden flex w-full min-w-0 min-h-56 justify-center text-xs",
					className
				)}
				{...props}
			>
...
function ChartTooltipContent({
	...
	payload?: Payload[];
	label?: string | number;
	labelFormatter?: (value: unknown, payload: Payload[]) => React.ReactNode;
	formatter?: (
		value: unknown,
		name: string,
		item: Payload,
	...
	return (
		<div
			className={cn(
				'grid min-w-32 items-start gap-1.5 rounded-lg border border-default-200 bg-background px-2.5 py-1.5 text-xs shadow-xl',
				className
			)}
		>
	...
	return (
		<div className={cn('flex items-center justify-center gap-4', verticalAlign === 'top' ? 'pb-3' : 'pt-3', className)}>
			{payload.map((item) => {
				...
				return (
					<div key={item.value} className={cn('flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted')}>
