...
export interface VerticalStepsProps extends Omit<React.HTMLAttributes<HTMLElement>, 'onChange'> {
	steps: VerticalStepConfig[];
	color?: StepColor;
	currentStep?: number;
	defaultStep?: number;
...
const VerticalSteps = React.forwardRef<HTMLElement, VerticalStepsProps>(
	(
		{
			color = 'primary',
			steps = [],
		...
		return (
			<LazyMotion features={domAnimation}>
				<nav ref={ref} aria-label="Progress" className={cn('w-full', className)} {...props}>
					<ol className="flex flex-col">
						{steps.map((step, stepIdx) => {
							...
							return (
								<li key={step.key} className="relative flex">
									<div className="flex flex-col items-center">
										<div
											className={cn('flex items-center justify-center', canClick && 'cursor-pointer')}
										...
										{!isLastStep && (
											<div className="relative h-12 w-0.5 bg-muted">
												<m.div
													className={cn('absolute inset-x-0 top-0 w-full', colors.bg)}
													initial={{ height: '0%' }}
